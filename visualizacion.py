...
if __name__ == "__main__":
    ...
    # Crear y entrenar
    env = GridWorld(size=6, num_trampas=8)
    agente = AgenteQLearning(
        num_estados=env.get_num_estados(),
        num_acciones=env.get_num_acciones(),
        alpha=0.1,
        gamma=0.9,
        epsilon=0.1
    )
    ...
    print("\n🎓 Entrenando agente...")
    stats = agente.entrenar(env, num_episodios=500, verbose=False)
    
    # Obtener mejor trayectoria
    trayectoria, _ = agente.obtener_mejor_trayectoria(env)
    
    # Crear visualizaciones
    print("\n📊 Creando visualizaciones...")
    viz = Visualizador()
    
    viz.graficar_recompensas(stats['recompensas'], guardar=True, mostrar=False)
    viz.graficar_pasos(stats['pasos'], guardar=True, mostrar=False)
    viz.graficar_tasa_exito(stats['exitos'], guardar=True, mostrar=False)
    viz.graficar_trayectoria(env, trayectoria, guardar=True, mostrar=False)
    viz.crear_dashboard(stats, env, trayectoria, guardar=True, mostrar=False)
    
    print("\n✅ ¡Todas las visualizaciones creadas!")
    print(f"📁 Revisa la carpeta: resultados/graficas/")
    print("=" * 60 + "\n")